...
3. `cd spark-bi`
4. `pip install -e .`
5. Åbn en notebook og genstart kernel for at sikre at den nye pakke er tilgængelig
   5.1 Klik på `Kernel` -> `Restart Kernel` i menuen i toppen
6. Du er nu klar til at køre notebooks. Først block i hver notebook skal køres for at initialisere forbindelse til Spark og S3, og derefter kan du køre "run all" for at køre hele notebook'en.
